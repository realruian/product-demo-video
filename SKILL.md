...
**它是自动的**——你只给一个能跑起来的项目，Claude 会：
- **自动理解产品**：读 `CLAUDE.md` / `README` / 路由，搞清每个页面是干什么的、产品的调性。
- **自动读代码生成文案**：根据每个页面的真实功能，自动写出对应场景的标题 / 副标题字幕与地址栏，不用你手写。
- **自动定位 logo**：在 `public/` 里找到你项目的真实矢量 logo，用在片头与收尾。

你只在几个关键点做选择题（画布比例、纳入哪些页面、节奏、声音）。

**前置 skill**：本 skill 依赖 [`remotion-best-practices`]（视频框架）和 [`agent-browser`]（截图）。处理 Remotion 代码前先加载 remotion-best-practices 拿领域知识。

## 何时用

用户要把一个跑得起来的 web 产品（自己的项目 / 给定目录）做成产品演示 / 宣传短视频。典型："给我们的产品做个演示视频"、"把这个页面做成竖屏宣传片"。
...
2. 列出页面路由（`find app -name page.tsx`），**读懂每个页面的功能**，自动归纳一条「用户旅程」叙事线（如「核心功能 A → B → C → 结果页」），并为每段自动拟好字幕文案。
...
- 地址栏随场景变（提升真实感）。
- 顶部 + 收尾**自动定位并使用项目真实 logo**（在 `public/` 里找 `icons/logo.svg` 之类的矢量字标，别用纯文字假冒）。
- 严格遵守**调性规范**（见下）。

### 阶段 5 — 声音 & 渲染
- **BGM（默认）**：Mixkit 免版权可商用音乐，[scripts/get-bgm.sh](scripts/get-bgm.sh) 下载+试听让用户选。整片铺底，首尾淡入淡出。
- **TTS 旁白（可选）**：[scripts/gen-tts.sh](scripts/gen-tts.sh)，走 AiHubMix/OpenAI 兼容 `/audio/speech`（gpt-4o-mini-tts 真人级，需用户 key）。每句对齐场景，场景时长按旁白长度。**别用 macOS `say`**（机械音，质量差）。
...
- **headless 浏览器没有麦克风/摄像头**：实时语音 / 视频类页面会卡在「准备 / 设备不可用」，截不到——直接走代码重建，别死磕。
- **本地 dev 数据库 schema 漂移**：代码加了 migration 但本地库没跑，会 500（如 `no such column: xxx`）。为截图 / 填数据，可 `ALTER TABLE ADD COLUMN` 补**缺失的列**（安全，不删数据），但要说明。
- **改本地数据库必须还原**：重置的测试密码、临时填充的示例数据，截完一律还原原值（先备份原 hash/JSON）。补的列可保留（本就是代码期望的）。
- **objectFit 别裁内容**：截图放进窗口用 `objectFit:"contain"` 完整显示；Ken Burns 放大要作用在外层窗口，不是图片，否则页面底部被裁。
- **填充示例数据时字段名要去代码确认**：前端读的字段名常与直觉不同（复数 / 下划线命名等），必须读渲染组件确认，别猜。
